...
export default function HierarchyMindMapPage() {
    ...
    const handleSearchResultClick = useCallback((result) => {
        const parentById = new Map(flatData.map(item => [item.id, item.parent_id]));
        const ancestors = new Set();
        let parentId = parentById.get(result.id);
        ...
        pendingFocusId.current = result.id;
        setCollapsedIds(prev => {
            const next = new Set(prev);
    ...
    }, [flatData, collapsedIds, handleToggle, selectedNodeId, searchResults, setNodes, setEdges, userRole]);
    ...
    return (
        <div className="relative h-full min-h-0 w-full overflow-hidden bg-slate-900/40">
